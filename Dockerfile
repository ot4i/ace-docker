...
ARG DOWNLOAD_URL=<Your downloaded location>/13.0.1.0-ACE-LINUX64-DEVELOPER.tar.gz
...
RUN mkdir -p /opt/ibm/ace-13 \
    ...
    --exclude ace-13.0.*.*/tools \
    --exclude ace-13.0.*.*/server/tools/ibm-dfdl-java.zip \
    --exclude ace-13.0.*.*/server/tools/proxyservlet.war \
    --exclude ace-13.0.*.*/server/bin/TADataCollector.sh \
    --exclude ace-13.0.*.*/server/transformationAdvisor/ta-plugin-ace.jar \
    --strip-components 1 \
...
# Install ACE v13.0.1.0 and accept the license
